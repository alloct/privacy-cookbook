...
travel unencrypted, which means whoever runs your network (and your ISP) gets a complete, timestamped list of every domain you contact, even when all
...
If you use a [VPN](/network/vpns/), use its resolvers while connected, sending DNS outside the tunnel recreates the leak the tunnel exists to close.
